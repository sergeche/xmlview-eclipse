...
import ru.chikuyonok.xv.ImageResource;

@SuppressWarnings("restriction")
public class XVBrowserEditor extends EditorPart implements
		IBrowserViewerContainer {
	
	public static final String WEB_BROWSER_EDITOR_ID = "ru.chikuyonok.xv.browser.editor"; //$NON-NLS-1$
	
	protected XVBrowser webBrowser;
	...
	public void createPartControl(Composite parent) {
		webBrowser = new XVBrowser(parent, 0);
		...
		if (!lockName) {
			PropertyChangeListener propertyChangeListener = new PropertyChangeListener() {
				public void propertyChange(PropertyChangeEvent event) {
					if (XVBrowser.PROPERTY_TITLE.equals(event.getPropertyName())) {
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		...
		if (input instanceof IPathEditorInput) {
			...
			ImageDescriptor id = ImageResource.getImageDescriptor(ImageResource.IMG_BROWSER_ICON);
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				if (WEB_BROWSER_EDITOR_ID.equals(editors[i].getId())) {
					...
					if (editor != null && editor instanceof XVBrowserEditor) {
						XVBrowserEditor webEditor = (XVBrowserEditor) editor;
			...
			page.openEditor(input, XVBrowserEditor.WEB_BROWSER_EDITOR_ID);
		} catch (Exception e) {
	...
	public boolean close() {
		...
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				result[0] = getEditorSite().getPage().closeEditor(XVBrowserEditor.this, false);
			}
		});
        return result[0];
	}
